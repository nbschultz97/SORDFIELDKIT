...
export default function App() {
  ...
  const chunkRatio = (() => {
    const stored = offline.status.storedChunks;
    const total = offline.status.totalChunks;
    if (total && total > 0) {
      return `${Math.min(stored, total)}/${total}`;
    }
    if (stored > 0) {
      return `${stored}/?`;
    }
  ...
  const tilesLabel = (() => {
    switch (offline.status.phase) {
      case "downloading":
        return `Tiles ${offline.progress}% (${chunkRatio})`;
      case "paused":
        return `Tiles paused ${offline.progress}% (${chunkRatio})`;
      case "ready":
        return `Tiles 100% (${chunkRatio})`;
      case "error":
        return "Tiles error";
      default:
        if (offline.hasCache) {
          return `Tiles cached (${chunkRatio})`;
        }
        return "Tiles live";
    }
  })();
