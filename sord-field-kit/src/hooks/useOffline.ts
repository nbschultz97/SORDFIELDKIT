      ...
      if (!cancelled) {
        ...
        if (blob) {
          setStatus((prev) => ({
            ...prev,
            phase: "ready",
            storedChunks: Math.max(prev.storedChunks, prev.totalChunks || 1),
            totalChunks: Math.max(prev.totalChunks, 1),
            bytesStored: prev.bytesStored || blob.size,
  ...
  const startCaching = useCallback(async () => {
    ...
    try {
      ...
      setStatus({
        ...
        totalChunks: estimatedChunks || storedChunks || 1,
